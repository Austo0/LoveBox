...
void GuiTask(void *pvParameters) {
 // Initialise the TFT display and touch screen
  tft.begin(); /* TFT init */
...
// Run the lvgl task handler to update the display
  while (1) 
  {
        lv_task_handler();
  }
...
void LvglInit()
{
    ...
    lv_init();  // Initialise the LVGL lib
...
static void lv_main()
{
    ...
    lv_obj_set_hidden(scr_home,false);      // Display the home screen 
    lv_obj_set_hidden(scr_settings,true);  // Hide the settings screen 
...
void UpdateTimeDateLabelText(String text)
{
  // Wifi Status text
  lv_label_set_text(label_time_date, text.c_str());
}
...
void UpdateWifiScanDropDown(String text)
{
    lv_dropdown_set_options(wifi_scan_list,text.c_str());
    lv_dropdown_set_text(wifi_scan_list, "Select SSID");
}
...
//Exit settings
static void BtnSettingsExitEventHandler(lv_obj_t * btn_wifi_scan, lv_event_t event)
{
    if(event == LV_EVENT_CLICKED)   // If the settings button is clicked
    {
        lv_obj_set_hidden(scr_settings,true);       // Hide the main screen 
        lv_obj_set_hidden(scr_home,false);  // Display the settings screen
    } 
}
...
void InitHomeScreen()
{
  ...
  // Text in write message button object
  label_time_date = lv_label_create(scr_home, NULL);
  lv_label_set_text(label_time_date, "Time - Date");
  lv_obj_align(label_time_date, NULL, LV_ALIGN_IN_TOP_RIGHT, 10, 2);
...
void InitSettingsScreen()
{
  ...
  lv_obj_set_pos(wifi_scan_list, 145, 5);  
  ...
  lv_obj_add_style(label_wifi_disconnect_btn, LV_LABEL_PART_MAIN,&heading_2_text_style);
    ...
    // Initialise save settings button object
  lv_obj_t *btn_save_settings = lv_btn_create(scr_settings,NULL);
  lv_obj_set_event_cb(btn_save_settings, BtnSettingsDisconnectWifiEventHandler);
  lv_obj_set_pos(btn_save_settings, 160, 205);
  lv_obj_set_size(btn_save_settings, 75, 25);
  lv_obj_add_style(btn_save_settings, LV_BTN_PART_MAIN, &nav_button_style); 

  // Text in save settings button object
  lv_obj_t *label_save_settings_btn = lv_label_create(btn_save_settings, NULL);
  lv_label_set_text(label_save_settings_btn, "Save");
  lv_obj_align(label_save_settings_btn, NULL, LV_ALIGN_CENTER, 0, 0);
  lv_obj_add_style(label_save_settings_btn, LV_LABEL_PART_MAIN,&heading_2_text_style);

    // Initialise exit settings button object
  lv_obj_t *btn_exit_settings = lv_btn_create(scr_settings,NULL);
  lv_obj_set_event_cb(btn_exit_settings, BtnSettingsExitEventHandler);
  lv_obj_set_pos(btn_exit_settings, 240, 205);
  lv_obj_set_size(btn_exit_settings, 75, 25);
  lv_obj_add_style(btn_exit_settings, LV_BTN_PART_MAIN, &nav_button_style); 

  // Text in exit settings button object
  lv_obj_t *label_exit_settings_btn = lv_label_create(btn_exit_settings, NULL);
  lv_label_set_text(label_exit_settings_btn, "Exit");
  lv_obj_align(label_exit_settings_btn, NULL, LV_ALIGN_CENTER, 0, 0);
  lv_obj_add_style(label_exit_settings_btn, LV_LABEL_PART_MAIN,&heading_2_text_style);
...
void InitViewMessagesScreen()
{
    
  scr_view_messages = lv_obj_create(scr, NULL);
  lv_obj_clean_style_list(scr_view_messages, LV_OBJ_PART_MAIN);
  lv_obj_set_style_local_bg_opa(scr_view_messages, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT,LV_OPA_COVER);
  lv_obj_set_style_local_bg_color(scr_view_messages, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT,LV_COLOR_NAVY);
  lv_obj_set_pos(scr_view_messages, 0, 0);
  lv_obj_set_size(scr_view_messages, LV_HOR_RES, LV_VER_RES);
